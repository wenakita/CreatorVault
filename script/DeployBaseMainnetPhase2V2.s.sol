...
///
/// Why:
/// - `UniversalBytecodeStore` v1 uses a single SSTORE2 pointer and cannot store >24KB creation bytecode.
/// - `CreatorOVault` creation bytecode is now ~29KB, so v1 `store.store()` reverts with `SSTORE2_WRITE_FAILED`.
/// - v2 store chunks large bytecode across multiple SSTORE2 pointers.
///
/// Run (broadcast):
...
/// Env:
///   PRIVATE_KEY=...
///   REGISTRY=... (optional; defaults to current Base defaults)
///   PROTOCOL_TREASURY=...
///   POOL_MANAGER=...
///   TAX_HOOK=...
///   CHAINLINK_ETH_USD=...
