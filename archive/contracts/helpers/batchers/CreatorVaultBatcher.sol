...
/**
 * ARCHIVED (legacy):
 * - This is the legacy one-tx deploy+launch batcher (`deployAndLaunch*`).
 * - The repo has cut over to the phased deployer (`CreatorVaultDeployer`, Phases 1–3).
 * - Kept only for historical reproducibility; safe to delete once no longer needed.
 */

/**
 * @title CreatorVaultBatcher
 * @author 0xakita.eth
 * @notice Batches vault activation and creator setup actions.
 * @dev Used by deployment scripts and onboarding flows.
 */
...
contract CreatorVaultBatcher is ReentrancyGuard, EIP712 {
    ...
    function _deployAndLaunch(
        ...
        IOwnableTransfer(result.oracle).transferOwnership(protocolTreasury);
