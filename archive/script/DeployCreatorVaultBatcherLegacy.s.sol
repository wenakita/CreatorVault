...
/**
 * ARCHIVED (legacy):
 * Deploy script for the legacy one-tx `CreatorVaultBatcher`.
 * The repo has cut over to the phased `CreatorVaultDeployer`.
 */
...
/**
 * @dev DEPRECATED: Deploy the legacy one-tx CreatorVaultBatcher on Base mainnet.
 *
 * This path is intentionally no longer supported by the frontend deploy UX nor the paymaster allowlist.
 * Keep this script only for historical deployments / reproducibility.
 */
contract DeployCreatorVaultBatcherLegacy is Script {
    ...
    function run() external {
        ...
        console2.log("Legacy CreatorVaultBatcher deployed:", address(batcher));
    }
}
