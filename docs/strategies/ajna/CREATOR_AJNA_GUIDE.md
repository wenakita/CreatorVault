...
./scripts/deploy/ajna/DEPLOY_CREATOR_AJNA.sh <TOKEN_ADDRESS> <VAULT_ADDRESS>

# Example for AKITA:
./scripts/deploy/ajna/DEPLOY_CREATOR_AJNA.sh \
...
The script uses default bucket **4156** (price = 1.0 quote per collateral).
...
# 3. Calculate Ajna bucket (approx)
# 50 Uniswap ticks ≈ 0.5% (≈ Ajna 1.005 step)
BUCKET=$((4156 - ($TICK / 50)))

# 4. Clamp to valid Ajna range (1..7388). Note: index 0 is invalid on Ajna pools.
...
# Set bucket (optional, default is 4156)
...
./scripts/deploy/ajna/DEPLOY_CREATOR_AJNA.sh $TOKEN $VAULT
...
./scripts/deploy/ajna/DEPLOY_CREATOR_AJNA.sh $TOKEN $VAULT  # Ajna lending
...
- [ ] Run `./scripts/deploy/ajna/DEPLOY_CREATOR_AJNA.sh <TOKEN> <VAULT>`
