...
Vault Periphery Repo: https://github.com/yearn/vault-periphery TokenizedStrategy Periphery: https://github.com/yearn/tokenized-strategy-periphery
...
The Yearn specific periphery contracts can be retrieved from that chains Role Manager
...
Each time a new Version of the Vault Factory is deployed it will get added to the Release Registry.
...
Deploy a custom Registry using the RegistryFactory
...
If set an accountant is called during each process_report with the relevant information for the strategies report. The accountant can hold any logic desired to charge performance fees, management fees, issue refunds, perform healthchecks etc.
...
To deploy a generic account use the Accountant Factory and then call set_accountant on the vault.
...
maxDebtUpdateLoss The value to send as maxLoss for a debt update. Default is 1 (0.01%).
...
You can deploy a single generic Debt Allocator using the Debt Allocator Factory for any V3 vault.
