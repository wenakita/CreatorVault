...
fn main() {
    ...
    println!("║      Forge CREATE2 Deployer (for scripts)              ║");
    println!("║      Pattern: 0x47...ea91e (FULL MATCH)                 ║");
    println!("╚══════════════════════════════════════════════════════════╝\n");

    // FORGE'S CREATE2 DEPLOYER (used in scripts with assembly create2)
    ...
    // Contract 1: EagleRegistry (FIRST - no dependencies, partial pattern)
    println!("═══════════════════════════════════════════════════════════");
    println!("CONTRACT 1/5: EagleRegistry [PARTIAL VANITY]");
    println!("═══════════════════════════════════════════════════════════\n");
    
    let registry_result = generate_vanity_registry(&deployer_bytes, deployer);
    let registry_addr = registry_result.address.clone();
    all_results.push(registry_result);
    
    println!("\n");

    // Contract 2: EagleShareOFT (PREMIUM - full pattern, needs Registry)
    println!("═══════════════════════════════════════════════════════════");
    println!("CONTRACT 2/5: EagleShareOFT [PREMIUM VANITY]");
    println!("═══════════════════════════════════════════════════════════\n");
    
    let oft_result = generate_vanity_oft(&deployer_bytes, deployer, &registry_addr);
    let oft_addr = oft_result.address.clone();
    all_results.push(oft_result);
    
    println!("\n");

    // Contract 3: EagleOVault (partial pattern 0x47...)
    println!("═══════════════════════════════════════════════════════════");
    println!("CONTRACT 3/5: EagleOVault [PARTIAL VANITY]");
    println!("═══════════════════════════════════════════════════════════\n");
    
    let vault_result = generate_vanity_vault(&deployer_bytes, deployer);
    let vault_addr = vault_result.address.clone();
    all_results.push(vault_result);
    
    println!("\n");

    // Contract 4: EagleVaultWrapper (partial pattern 0x47...)
    println!("═══════════════════════════════════════════════════════════");
    println!("CONTRACT 4/5: EagleVaultWrapper [PARTIAL VANITY]");
    println!("═══════════════════════════════════════════════════════════\n");
    
    let wrapper_result = generate_vanity_wrapper(&deployer_bytes, deployer, &vault_addr, &oft_addr);
    all_results.push(wrapper_result);
    
    println!("\n");

    // Contract 5: CharmStrategyUSD1 (partial pattern 0x47...)
    println!("═══════════════════════════════════════════════════════════");
    println!("CONTRACT 5/5: CharmStrategyUSD1 [PARTIAL VANITY]");
    println!("═══════════════════════════════════════════════════════════\n");
    
    let strategy_result = generate_vanity_strategy(&deployer_bytes, deployer, &vault_addr);
...
fn generate_vanity_registry(deployer_bytes: &[u8], deployer: &str) -> VanityResult {
    let artifact_path = "../out/EagleRegistry.sol/EagleRegistry.json";
    let artifact_content = fs::read_to_string(artifact_path)
        .expect("Failed to read EagleRegistry artifact");
    ...
    let owner = "0x7310Dd6EF89b7f829839F140C6840bc929ba2031"; // Your wallet
    ...
    find_vanity_partial_pattern(deployer_bytes, &init_code_hash, "EagleRegistry", deployer)
}

fn generate_vanity_oft(deployer_bytes: &[u8], deployer: &str, registry: &str) -> VanityResult {
    ...
    let name = "Eagle";
    let symbol = "EAGLE";
    let delegate = "0x7310Dd6EF89b7f829839F140C6840bc929ba2031"; // Your wallet
...
fn generate_vanity_wrapper(deployer_bytes: &[u8], deployer: &str, vault: &str, oft: &str) -> VanityResult {
...
fn generate_vanity_strategy(deployer_bytes: &[u8], deployer: &str, vault: &str) -> VanityResult {
