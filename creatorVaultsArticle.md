...
CreatorVault is a Base-native creator finance layer that turns Creator Coins into composable onchain economies. The protocol combines ERC-4626 vaults, omnichain share tokens, a fee-splitting gauge, and a lottery system, while anchoring governance and incentives in vote-escrowed liquidity (ve■4626). The end result is a system where creator tokens become programmable ecosystems: deposits mint vault shares, shares wrap into omnichain tokens, DEX trading fees flow into a gauge, and ve-style voting steers probability and rewards. Every new creator Content Coin launch can expand the fee surface area and shared earnings, potentially lifting overall vault APR as more activity routes into the shared reward flows. A key part of this flow is rerouting each CreatorCoin’s `payoutRecipient` to a per-vault `PayoutRouter` so creator earnings are converted into vault shares and burned, raising PPS within that vault only.[^1][^30]
...
### Payout recipient routing for vault-only earnings

When a creator vault is deployed, the creator coin’s `payoutRecipient` can be set to a `PayoutRouter` tied to that vault. The router deposits creator earnings into the vault, wraps them into the share token, and schedules a burn to increase PPS for that specific vault’s holders, without routing value to other vaults or the gauge’s lottery split.[^31]
...
### Charm (Uniswap V3 LP via CreatorCharmStrategy)

Charm provides automated V3 LP management, giving the vault exposure to trading fees and price discovery. The CreatorCharmStrategy is built for **single-sided creator-coin deposits**, handling the rebalancing and range management needed for V3 liquidity. This enables the vault to participate in deep trading venues without forcing users to provide both assets manually.[^20]

### Ajna (Lending via AjnaStrategy)
...
- **Liquidity resilience**: LP strategies can suffer during volatile rebalances, while lending can still produce steady yield when borrowing demand is healthy.[^19]
- **More robust allocation knobs**: The strategy deployment flow allows explicit weightings (e.g., Charm ~69% and Ajna ~21.39% as seen in multi-strategy allocation docs). This lets governance tune risk/return without rebuilding the vault.[^22]
- **Single-sided UX maintained**: Charm’s integration specifically supports creator-only deposits, keeping the user flow simple while still accessing dual-asset liquidity on Uniswap V3.[^20]

The result is a vault that can earn both **trading fees** and **lending yield**, balancing fee volatility against lending stability, and making creator economies more resilient to market regime shifts.[^19]
...
- **Multi-strategy allocation** balances LP fee capture with lending yield, improving resilience across market regimes.[^19]
...
[^19]: docs/architecture/STRATEGY_ARCHITECTURE.md (multi-strategy rationale)
[^20]: docs/strategies/univ3/CREATOR_CHARM_GUIDE.md (Charm strategy behavior)
...
[^30]: contracts/helpers/batchers/CreatorVaultBatcher.sol (payoutRecipient routing on deployment)
[^31]: contracts/helpers/routers/PayoutRouter.sol (payoutRecipient routing + burn-stream)