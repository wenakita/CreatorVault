...
contract VaultStrategyBootstrapper {
    ...
    /**
     * @notice Same as `finalize`, but deploys the canonical `CharmAlphaVault` (not `CharmAlphaVaultSimple`).
     * @dev Trade-off: Charm vault governance will be left as "pending" until `finalOwner` calls
     *      `CharmAlphaVault(charmVault).acceptGovernance()` in a follow-up transaction.
     *
     *      This keeps the deployed bytecode identical to the original Charm vault implementation.
     */
    function finalizeFullCharmVault(
        address vault,
        address wrapper,
        address gaugeController,
        address creatorToken,
        ...
        // 1) Deploy yield strategies (Charm + Ajna) and any required pool/vault plumbing.
        StrategyDeploymentBatcher batcher = new StrategyDeploymentBatcher();
        StrategyDeploymentBatcher.DeploymentResult memory result = batcher.batchDeployStrategiesFullCharmVault(
            creatorToken,
            usdc,
            vault,
            ajnaFactory,
