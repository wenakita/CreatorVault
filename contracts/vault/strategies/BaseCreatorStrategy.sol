...
abstract contract BaseCreatorStrategy is IStrategy, Ownable, ReentrancyGuard {
        ...
        returns (uint256 withdrawn) 
    {
        ...
        uint256 localAssetBalance = ASSET.balanceOf(address(this));
        
        if (localAssetBalance >= amount) {
            // Enough locally
            withdrawn = amount;
        } else {
            // Need to free funds from yield source
            uint256 needed = amount - localAssetBalance;
            uint256 freed = _freeFunds(needed);
            withdrawn = localAssetBalance + freed;
        }
