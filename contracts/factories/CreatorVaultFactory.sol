...
import {CreatOVault} from "../CreatOVault.sol";
import {CreatorOVaultWrapper} from "../CreatorOVaultWrapper.sol";
import {CreatorShareOFT} from "../CreatorShareOFT.sol";
import {CreatorGaugeController} from "../CreatorGaugeController.sol";
 ...
 *      1. CreatOVault - ERC-4626 vault for Creator Coin
 ...
 *      Creator Coin: akita
 *      Vault Share: "AKITA Vault" (vAKITA) - deposit receipt
 *      Wrapped Share: "Wrapped AKITA Share" (wsAKITA) - cross-chain OFT
 * 
 * @dev TOKEN FLOW:
 *      akita → deposit → vAKITA → wrap → wsAKITA (trades on DEX)
 */
contract CreatorVaultFactory is Ownable {
     ...
     * @return vault CreatOVault address
     ...
     * 
     * @dev TOKEN FLOW:
     *      akita → deposit → vAKITA → wrap → wsAKITA (cross-chain)
     */
     ...
     * @param _vaultSymbol Vault token symbol (e.g., "vAKITA")
    ...
    function _deployVault(
        ...
        CreatOVault vault = new CreatOVault(
            _creatorCoin,
            _name,
            _symbol,
            _owner
    ...
    function _deployGaugeController(
        ...
        CreatorGaugeController(controller).setVault(_vault);
        CreatorGaugeController(controller).setWrapper(_wrapper);
    ...
    function _configureAllPermissions(
        ...
        CreatOVault(_vault).setGaugeController(_gaugeController);
        
        // Whitelist the wrapper for deposits
        CreatOVault(_vault).setWhitelist(_wrapper, true);
    }
