 ...
 *      When someone trades the creator's tokens, 6.9% fees flow here from TWO sources:
 *      
 *      SOURCE 1: ShareOFT Buy Fees (6.9% on wsAKITA purchases)
 *      SOURCE 2: V4 Tax Hook (6.9% on wsAKITA/ETH LP swaps)
 *      
 *      Fee Distribution:
 *      
 *      1. LOTTERY (90%): Jackpot pool for swap-to-win lottery
 *         → Every swap is a lottery ticket
 *         → veToken holders get up to 2.5x boost on win probability
 *      
 *      2. CREATOR (5%): Creator's treasury earnings
 *      
 *      3. PROTOCOL (5%): CreatorVault multisig (platform fee)
 *      
 *      4. BURN (0% default): Optional - can burn to increase PPS
 * 
 * @dev TOKEN FLOW (OFT fees):
 *      wsAKITA (OFT) → unwrap → vAKITA (vault shares) → distribute
 * 
 * @dev TOKEN FLOW (V4 Tax Hook fees):
 *      WETH → swap → akita → deposit → vAKITA → distribute
 *                                        ↓
 *               ┌────────────────────────┼────────────────────────┐
 *               ↓                        ↓                        ↓
 *             90%                       5%                       5%
 ...
 *      - 0%  → Burn (disabled by default)
 * 
 * @dev V4 TAX HOOK:
 *      The existing tax hook at 0xca975B9dAF772C71161f3648437c3616E5Be0088 on Base
 *      can be configured to send 6.9% of swap fees to this contract as WETH.
 */
