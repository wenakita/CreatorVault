 ...
 * @notice Adapts any ERC-4626 vault into the CreatorOVault `IStrategy` interface.
 * @dev The strategy holds ERC-4626 shares and reports value via `convertToAssets(sharesHeld)`.
 *
 * Safety goals (CreatorOVault withdrawal queue friendly):
 * - `getTotalAssets()` MUST NOT revert (best-effort conversion if the 4626 vault misbehaves).
 * - `withdraw()` should be best-effort and NEVER revert due to the 4626 vault; it returns what it can.
 *
 * Operational goals:
 * - Optional idle buffer (bps) to reduce reliance on the 4626 vault for routine withdrawals.
 * - No-op `harvest()` (CreatorOVault captures profit via totalAssets deltas on report()).
 */
