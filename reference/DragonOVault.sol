...
import {IStrategy} from "../../interfaces/vaults/IStrategy.sol";

/**
 * @title DragonOVault
 * @author 0xakita.eth
 * @notice ERC-4626 vault for omniDRAGON with full strategy support
 * 
 * @dev ARCHITECTURE:
 *      - Fully ERC-4626 compliant vault
 *      - Deposit omniDRAGON → mint dragonVAULT shares
 *      - Deploy idle assets to yield strategies
 *      - Profit unlocking prevents PPS manipulation
 *      - Same bytecode + args = same CREATE2 address on all chains
 * 
 * @dev STRATEGY SYSTEM:
 ...
 *      - _dragon: omniDRAGON (0x69cCC6724eE86868986c8605f286895cE7777777)
 *      - _owner: deployer
 */
contract DragonOVault is ERC4626, Ownable, ReentrancyGuard {
    ...
    /**
     * @notice Minimum first deposit to prevent dust manipulation
     * @dev Must be large enough that donation attacks are economically infeasible
     *      With 69M DRAGON per chain, 10,000 DRAGON = 0.015% of supply
     *      Attack cost would be ~10M DRAGON (14.5% of supply) to steal 1%
     * @custom:security Prevents "dust deposit → inflate → drain" attack vector
     */
    uint256 public constant MINIMUM_FIRST_DEPOSIT = 10_000e18; // 10,000 DRAGON minimum
    ...
    /// @notice Current DRAGON balance held directly by vault
    uint256 public dragonBalance;
    ...
    /// @notice Keep this much DRAGON idle for redemptions
    ...
    uint256 public largeWithdrawalThreshold = 100_000e18; // 100k DRAGON
    ...
    /// @notice Minimum DRAGON to keep idle for fast redemptions
    /// @dev Based on Yearn V3: minimum_total_idle pattern
    uint256 public minimumTotalIdle = 10_000e18; // 10k DRAGON default
    ...
    event BalancesSynced(uint256 dragonBalance);
    ...
    /**
     * @notice Deploy DragonOVault with same address on all chains
     * @param _dragon omniDRAGON address (same on all chains)
    ...
    constructor(
        ...
        ERC20("Dragon OVault", "dragonOVAULT") 
        ERC4626(IERC20(_dragon)) 
        Ownable(_owner) 
    {
        if (_dragon == address(0)) revert ZeroAddress();
    ...
    /**
     * @notice Deposit DRAGON into vault
        ...
        returns (uint256 shares) 
    {
        ...
        // With _decimalsOffset = 3, first deposit gives shares = assets * 1000 (virtual offset)
        // Subsequent deposits should be approximately 1:1000 (shares:assets in wei terms)
        // We allow up to 10,000x to account for virtual offset + some variance
        if (!isFirstDeposit && shares > assets * 10_000) {
            revert InflationAttackDetected(assets, shares);
        }
        
        // Pull DRAGON
        DRAGON.safeTransferFrom(msg.sender, address(this), assets);
        dragonBalance += assets;
        
        // Mint shares
        _mint(receiver, shares);
        
        // SECURITY: Track deposit block for flash loan protection (yETH mitigation)
        lastDepositBlock[receiver] = block.number;
        
        // SECURITY: Verify price didn't change dramatically (prevents manipulation)
        // Skip check for first deposit since price changes from default to actual
        if (!isFirstDeposit) {
        ...
        returns (uint256 assets)
    {
        ...
        // With _decimalsOffset = 3, shares = assets * 1000 for first deposit
        // Allow up to 10,000x for subsequent deposits with variance
        if (!isFirstDeposit && shares > assets * 10_000) {
            revert InflationAttackDetected(assets, shares);
        }
    ...
    /**
     * @notice Redeem shares for DRAGON
     * @dev SYNCHRONOUS - Transfers immediately for small amounts
     *      Large withdrawals must be queued for yETH-style protection
        ...
        returns (uint256 assets)
    {
        ...
        // This prevents same-tx deposit→manipulate→withdraw attacks (yETH mitigation)
        ...
        // SECURITY: Large withdrawals must be queued (yETH mitigation)
        ...
        // Ensure we have enough DRAGON
        _ensureDragon(assets);
        
        dragonBalance -= assets;
        DRAGON.safeTransfer(receiver, assets);
        
        emit Withdraw(msg.sender, receiver, owner_, assets, shares);
    }
    
    /**
     * @notice Withdraw exact DRAGON amount
     * @dev SYNCHRONOUS - Transfers immediately for small amounts
     *      Large withdrawals must be queued for yETH-style protection
     * @custom:security Flash loan protected - cannot withdraw same block as deposit
     */
    ...
    // =================================
    // LARGE WITHDRAWAL QUEUE (yETH Protection)
    // =================================
     ...
     * @param receiver Address to receive DRAGON when claimed
     */
    ...
    // =================================
    // ENSURE DRAGON HELPER
    // =================================
    
    /**
     * @notice Ensure vault has enough DRAGON for redemptions
     * @dev Withdraws from strategies if needed
     */
    function _ensureDragon(uint256 dragonNeeded) internal {
        if (dragonBalance >= dragonNeeded) return;
        
        uint256 deficit = dragonNeeded - dragonBalance;
        
        // Withdraw from strategies
        _withdrawFromStrategies(deficit);
        
        if (dragonBalance < dragonNeeded) {
    ...
    function addStrategy(address strategy, uint256 weight, bool addToQueue) public onlyManagement {
        ...
        require(IStrategy(strategy).dragon() == address(DRAGON), "Wrong asset");
    ...
    function _deployToStrategies() internal {
        ...
        uint256 deployable = dragonBalance > minIdle 
            ? dragonBalance - minIdle 
            : 0;
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (activeStrategies[strategy] && strategyWeights[strategy] > 0) {
                ...
                if (amount > dragonBalance) amount = dragonBalance;
                
                if (amount > 0) {
                    uint256 currentDebt = strategyDebt[strategy];
                    dragonBalance -= amount;
                    DRAGON.forceApprove(strategy, amount);
    ...
    function _assessUnrealisedLoss(
        ...
        // loss_share = assets_needed * (1 - strategy_assets / current_debt)
    ...
    function _autoAllocateToStrategy() internal {
        ...
        uint256 minIdle = minimumTotalIdle > deploymentThreshold ? minimumTotalIdle : deploymentThreshold;
        if (dragonBalance <= minIdle) return;
        
        uint256 toAllocate = dragonBalance - minIdle;
        if (toAllocate == 0) return;
        
        uint256 currentDebt = strategyDebt[firstStrategy];
        
        dragonBalance -= toAllocate;
        DRAGON.forceApprove(firstStrategy, toAllocate);
    ...
    function tend() external nonReentrant onlyKeepers {
        if (dragonBalance > deploymentThreshold && totalStrategyWeight > 0) {
            _deployToStrategies();
        }
    }
    ...
    function tendTrigger() external view returns (bool) {
        return dragonBalance > deploymentThreshold 
     ...
     *      When true, deposits automatically deploy to first strategy
     */
     ...
     *      Governance can buy strategy debt at 1:1 for asset
     *      Used in emergencies instead of force-revoking to avoid loss reporting
    ...
    function buyDebt(address strategy, uint256 amount) external nonReentrant onlyDebtPurchaser {
        ...
        // Buyer sends DRAGON to vault
        DRAGON.safeTransferFrom(msg.sender, address(this), _amount);
        dragonBalance += _amount;
    ...
    function emergencyWithdraw(uint256 amount, address to) external onlyEmergencyAuthorized {
        ...
        if (amount > 0) {
            DRAGON.safeTransfer(to, amount);
        }
        
        dragonBalance = DRAGON.balanceOf(address(this));
     ...
     * @dev yETH exploit mitigation - prevent same-block deposit/withdraw attacks
    ...
    function syncBalances() external onlyManagement {
        uint256 actual = DRAGON.balanceOf(address(this));
        dragonBalance = actual;
    ...
    function rescueToken(address token, uint256 amount, address to) external onlyOwner {
        if (token == address(DRAGON)) {
            revert("Use emergency functions for DRAGON");
        }
        if (to == address(0)) revert ZeroAddress();
